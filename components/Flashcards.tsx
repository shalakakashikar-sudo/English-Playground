...
const Flashcards: React.FC<FlashcardsProps> = ({ initialData, reviewCount }) => {
  ...
  const renderContent = () => {
    ...
    if (allCardsReviewed) {
        return (
            <div className="h-48 flex flex-col items-center justify-center text-dark-brown/70 dark:text-cream/70 bg-dark-brown/5 dark:bg-slate-900/50 rounded-2xl">
            ...
            <div className="flashcard-inner">
              <div className="flashcard-front p-3 sm:p-4">
                {card.status === 'needs_review' && (
                  <div className="absolute top-3 right-3 group" aria-label="This card needs review">
                    <div className="relative flex items-center justify-center h-4 w-4">
                      <div className="absolute -inset-1 bg-rose-500 rounded-full blur opacity-75 animate-pulse"></div>
                      <div className="relative h-2.5 w-2.5 bg-rose-500 rounded-full"></div>
                    </div>
                ...
                <div className="absolute top-4 left-4 flex items-center gap-1.5 text-xs uppercase font-semibold text-dark-brown/40 dark:text-cream/40">
                    {card.type === 'word' ? <WordIcon /> : <IdiomIcon />}
                    <span>{card.type}</span>
                </div>
                <button
                    onClick={(e) => handleSpeak(e, card.term)}
                    className="absolute bottom-2 right-2 p-2 rounded-full text-dark-brown/40 hover:bg-dark-brown/10 hover:text-dark-brown focus:outline-none focus:ring-2 focus:ring-dark-brown/50 dark:text-cream/40 dark:hover:bg-cream/10 dark:hover:text-cream dark:focus:ring-cream/50 transition-colors"
  ...
  return (
    <div className="bg-dark-brown/5 dark:bg-slate-900/50 p-4 sm:p-6 rounded-2xl mb-8 w-full">
